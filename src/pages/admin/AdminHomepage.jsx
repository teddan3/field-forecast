...
const empty = { section_name: '', section_title: '', section_subtitle: '', content: '', image: '', cta_text: '', cta_link: '', status: 'active', display_order: 0 };
...
export default function AdminHomepage() {
  ...
  const load = async () => {
    const s = await base44.entities.HomepageSection.list('display_order');
    setSections(s); setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const handleSave = async () => {
    setSaving(true);
    if (editing) await base44.entities.HomepageSection.update(editing.id, form);
    else await base44.entities.HomepageSection.create(form);
            ...
            <div key={s.id} className="flex items-center gap-4 bg-sidebar-accent rounded-xl border border-sidebar-border px-4 py-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs text-sidebar-foreground/40">{s.section_name}</span>
                ...
                <div className="text-sm font-medium text-sidebar-foreground">{s.section_title}</div>
                {s.section_subtitle && <div className="text-xs text-sidebar-foreground/50 truncate">{s.section_subtitle}</div>}
              </div>
              <span className="text-xs text-sidebar-foreground/40">Order: {s.display_order}</span>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2">
            ...
            <div><Label>Title</Label><Input value={form.section_title} onChange={e => setForm(p => ({...p, section_title: e.target.value}))} /></div>
            <div><Label>Subtitle</Label><Input value={form.section_subtitle} onChange={e => setForm(p => ({...p, section_subtitle: e.target.value}))} /></div>
            <div><Label>Content</Label><Textarea rows={3} value={form.content} onChange={e => setForm(p => ({...p, content: e.target.value}))} /></div>
            <div>
              <Label>Image</Label>
              <div className="mt-1 space-y-2">
                <input type="file" accept="image/*" onChange={handleImageUpload} className="text-sm text-sidebar-foreground/70" />
                {uploading && <p className="text-xs text-sidebar-foreground/50">Uploading...</p>}
                {form.image && <img src={form.image} alt="" className="h-24 w-full object-cover rounded-lg" />}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
